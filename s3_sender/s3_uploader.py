...
class S3Uploader:
    ...
    def get_s3_uploaded_object_md5sum(self, resource_name: str) -> str:
        ...
        try:
            md5sum = self.s3.head_object(Bucket=self.bucket_name, Key=resource_name)[
                "ETag"
            ][1:-1]
    ...
    def validate_object_lock(self) -> bool:
        ...
        try:
            lock_config = self.s3.get_bucket_object_lock_configuration(Bucket=self.bucket_name)
            ...
            current_mode = lock_config['ObjectLockConfiguration']['Rule']['DefaultRetention']['Mode']
            current_days = int(lock_config['ObjectLockConfiguration']['Rule']['DefaultRetention']['Days'])
    ...
    def upload_file(self, filepath: Path, s3_key: str) -> bool:
        ...
        self.s3.upload_file(Filename=str(filepath), Bucket=self.bucket_name, Key=s3_key)
        ...
        if original_md5 == uploaded_md5:
            ...
            raise botocore.exceptions.ClientError(f"File {filepath.name} upload failed due to MD5 hash mismatch.")
